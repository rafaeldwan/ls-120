...
class Dealer < Participant
  ...
  def choice(deck, player_total)
    loop do
      if total < HIT_UNTIL || ( total <= player_total && total < PLAY_TO ) 
 ...
 attr_reader :player, :dealer
  ...
  def start
    ...
    loop do
      ...
      show_initial_cards
      player_turn
      dealer_turn unless @player.busted? || @player.has_winning_score?
      result
  ...
  def player_turn
    loop do
      ...
      @player.display_hand
      break if @player.busted? || @player.has_winning_score?
    end
  
    if @player.has_winning_score?
      puts "#{player.score}. DEALER CANNOT BEAT THAT AND AT BEST CAN TIE."
    end
    sleep(0.3)
  end
  ...
  def display_result(result)
    sleep(0.5)
    case result
    when :player_bust then puts "YOU BUSTED. IT IS COMMON. YOU LOSE"
    when :dealer_bust then puts "DEALER BUSTED. YOU HAVE WON"
    when :tie then puts "UNCOMMON BUT NOT REMARKABLE: A TIE"
    when :dealer then puts "YOU WERE BEATEN BY THE DEALER IN ACCORDANCE WITH THE RULES"
    end
  end
